                    ...
                    const response = await model.generateContent({
                        contents: [{
                            ...
                            parts: [
                                ...
                                { text: "请注意，用户上传的翡翠图片是有特定拍摄意图和严格顺序的：\n1. 第一张图片：自然光正面照（主要用于观察并评估其：整体种水水头、主色调分布、雕工与器型品相）。\n2. 第二张图片：强光透射照（如果有，主要用于观察并评估其：内部晶体纤维交织度、天然棉絮的多寡、石纹以及有害的微裂缝）。\n3. 第三张图片：微距反射照（如果有，主要用于观察并评估其：表面反光出的苍蝇翅/翠性，严密筛查是否存在强酸洗胶留下的蜘蛛网状酸蚀网纹以判定真伪）。\n\n请严格结合这些特定的微观与宏观角度，对照行业标准案例进行靶向深度分析，绝对不要漏掉任何一个特定视角的细节，并返回符合要求的 JSON 报告。" }
                            ]
                        }],
                        generationConfig: { 
